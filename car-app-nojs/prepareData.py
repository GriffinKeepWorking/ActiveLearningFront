import os
from subprocess import Popen, PIPE, STDOUT
import argparse
import glob
from os.path import join
import yaml
import random



def parse_arguments():
    parser = argparse.ArgumentParser(description='prepare data')
    parser.add_argument('--input', '-i', type=str, nargs='+',
                        help='path to the images folder')
    parser.add_argument('--tar-dir', '-t', type=str, default='',
                        help='target folder to save the result')

    # Specify S3 bucket path
    parser.add_argument('--s3', type=str, default='',
                        help='bucket-name/subfolder')

    # Output webroot. Ignore it if you are using s3
    parser.add_argument('--web-root', '-w', type=str, default='',
                        help='webroot used as prefix in the yaml file')

    args = parser.parse_args()
    return args

def prepare_data(args):

    if args.s3:
        s3_setup(args)

    if type(args.input) == list:

        for i in range(len(args.input)):
            vf = args.input[i]

            if os.path.isdir(vf):
                print('Loading all existing images in folder {}'.format(vf))
                args.tar_dir = vf

            elif os.path.isfile(vf):
                if not os.path.exists(vf):
                    print('[WARNING] {} does not exist'.format(vf))
                    continue

                cmd = "ffmpeg -i {} -r {} -qscale:v 2 {}/{}-%07d.jpg".format(
                    vf, args.fps, args.tar_dir,
                    os.path.splitext(os.path.split(vf)[1])[0]
                )
                print('[RUNNING]', cmd)
                p = Popen(cmd, shell=True, stdout=PIPE, stderr=STDOUT)
                p.wait()
            else:
                print('[ERROR] Invalid `input` value `{}`. Neither file '
                      'nor directory '.format(vf))
                return None

    # create the yaml file
    file_list = sorted(glob.glob(join(args.tar_dir, '*.JPEG')))

    yaml_items = [
        {'url':os.path.abspath(img) if not args.web_root else
            join(args.web_root, os.path.basename(img))}
        for img in file_list]

    # total list
    output = join(args.tar_dir, 'image_list.txt')
    with open(output, 'w') as f:
        yaml.dump(yaml_items, f, default_flow_style=False)
	
    # divide into random five lists
    random.shuffle(yaml_items)
    n = len(yaml_items) // 10
    temp = [yaml_items[i * n:(i + 1) * n] for i in range((len(yaml_items) + n - 1) // n )] 
    temp1 = join(args.tar_dir, 'task1.txt')
    temp2 = join(args.tar_dir, 'task2.txt')
    temp3 = join(args.tar_dir, 'task3.txt')
    temp4 = join(args.tar_dir, 'task4.txt')
    temp5 = join(args.tar_dir, 'task5.txt')
    temp6 = join(args.tar_dir, 'task6.txt')
    temp7 = join(args.tar_dir, 'task7.txt')
    temp8 = join(args.tar_dir, 'task8.txt')
    temp9 = join(args.tar_dir, 'task9.txt')
    temp10 = join(args.tar_dir, 'task10.txt')
    with open(temp1, 'w') as f:
        yaml.dump(temp[0], f, default_flow_style=False)
    with open(temp2, 'w') as f:
        yaml.dump(temp[1], f, default_flow_style=False)
    with open(temp3, 'w') as f:
        yaml.dump(temp[2], f, default_flow_style=False)
    with open(temp4, 'w') as f:
        yaml.dump(temp[3], f, default_flow_style=False)
    with open(temp5, 'w') as f:
        yaml.dump(temp[4], f, default_flow_style=False)    
    with open(temp6, 'w') as f:
        yaml.dump(temp[5], f, default_flow_style=False)
    with open(temp7, 'w') as f:
        yaml.dump(temp[6], f, default_flow_style=False)
    with open(temp8, 'w') as f:
        yaml.dump(temp[7], f, default_flow_style=False)
    with open(temp9, 'w') as f:
        yaml.dump(temp[8], f, default_flow_style=False)
    with open(temp10, 'w') as f:
        yaml.dump(temp[9], f, default_flow_style=False)  
    # upload to s3 if needed
    if args.s3:
        upload_files_to_s3(args)

    return output


def upload_files_to_s3(args):
    import boto3
    s3 = boto3.resource('s3')
    bn = args.bucket_name
    file_list = glob.glob(join(args.tar_dir, '*'))
    for f in file_list:
        try:
            s3.Bucket(bn).upload_file(f, join(args.s3_folder,
                                              os.path.basename(f)),
                                      ExtraArgs={'ACL': 'public-read'})
        except Exception as e:
            print('[ERROR] s3 bucket is not properly configured')
            print('[ERROR]', e)
            break


def s3_setup(args):
    import boto3

    args.bucket_name, args.s3_folder = args.s3.split('/')[0], '/'.join(
        args.s3.split('/')[1:])
    s3 = boto3.resource('s3')
    bn = args.bucket_name
    region = s3.meta.client.get_bucket_location(
        Bucket=bn)['LocationConstraint']

    args.web_root = join('https://s3-{}.amazonaws.com'.format(region),
                         bn, args.s3_folder)


def main():
    args = parse_arguments()
    if args.tar_dir:
        os.makedirs(args.tar_dir, exist_ok=True)

    output = prepare_data(args)
    if output is not None:
        print('[SUCCESS] The configuration file saved at {}'.format(output))

if __name__ == '__main__':
    main()
